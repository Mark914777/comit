...
from telegram.ext import (Application, CommandHandler, ContextTypes, MessageHandler, filters, CallbackContent, CallbackQueryHandler)
...
async def mode(Update, content: CallbackContent):
    keyboard = [
        [InlineKeyboardButton("GPT-3.5", callback_data="1")]
        [InlineKeyboardButton("Stable Diffusion", callback_data="2")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.text("Выбери нейронку!", reply_markup=reply_markup)

async def button(update: Update, content: CallbackContent):
    pandora = shelve.open("pandora")
        ...
        for button in row:
            if button.callback_data == mode_gpt:
              ...
              row[row.index(button)] = new_button
                ...
                await query.edit_massage_text(
                    text="Можешь сменить нейронку",
